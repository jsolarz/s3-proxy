...
module.exports = function (options) {
  const s3 = new AWS.S3(assign(awsConfig(options), pick(options, 'endpoint', 's3ForcePathStyle')));

  function listKeys(req, res, next) {
    const folderPath = req.originalUrl.substr(req.baseUrl.length);
  ...
  function getObject(req, res, next) {
    ...
    let s3Key = options.removeBucketFromObjectUrl
      ? decodeURIComponent(
        req.originalUrl.substr(req.baseUrl.length + 1 + options.bucket.length + 1),
      )
      : decodeURIComponent(req.originalUrl.substr(req.baseUrl.length + 1));
    ...
    console.log('get s3 object with key %s', s3Params.Key);
    ...
    console.log('read stream %s', s3Params.Key);

    let readStream = s3Request.createReadStream().on('error', (err) => {
  ...
  return function (req, res, next) {
    ...
    if (req.path === '/' && options.handleIndexHtml) getObject(req, res, next);
